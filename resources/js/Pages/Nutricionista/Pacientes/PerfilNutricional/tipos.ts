...
export interface RecomendacionNutricionalExperta {
    ...
    id_requerimiento_nutricional?: number | null;
...
export interface IngredienteRecetaPlan { id_receta_alimento:number; id_alimento:number; cantidad:number|string; unidad:string; observaciones?:string|null; alimento?:{id_alimento:number;nombre:string;unidad_base?:string|null}|null }
export interface CatalogoReceta { id_receta:number; nombre:string; imagen_url?:string|null; tipo_comida:string; porciones:number; preparacion?:string|null; calorias_totales:number|string; proteinas_totales:number|string; carbohidratos_totales:number|string; grasas_totales:number|string; fibra_total:number|string; receta_alimentos?:IngredienteRecetaPlan[] }
...
export interface ResumenAdherencia { comidas_totales:number; completadas:number; parciales:number; no_realizadas:number; reemplazadas:number; pendientes:number; registradas?:number; evaluables?:number; sin_registro_vencidas?:number; porcentaje_adherencia:number }
