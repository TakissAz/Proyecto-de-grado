...
const CAMPOS: { name: string; label: string; placeholder: string }[] = [
    { name: 'alergias', label: 'Alergias alimentarias', placeholder: 'Ej: maní, mariscos, lácteos, gluten...' },
    { name: 'intolerancias', label: 'Intolerancias', placeholder: 'Ej: lactosa, fructosa, FODMAP...' },
    { name: 'alimentos_restringidos', label: 'Alimentos restringidos', placeholder: 'Ej: azúcar refinada, sodio alto, grasas trans...' },
    { name: 'alimentos_no_tolerados', label: 'Alimentos no tolerados', placeholder: 'Ej: cebolla, repollo, legumbres...' },
    { name: 'alimentos_rechazados', label: 'Alimentos rechazados', placeholder: 'Ej: hígado, vísceras, pescado...' },
];

export default function ModalRestricciones({ abierto, cerrar, registro, pacienteId }: Props) {
    const id = registro?.id_restriccion_alimentaria;
    const url = `/nutricionista/pacientes/${pacienteId}/perfil-nutricional/restricciones${id ? `/${id}` : ''}`;
            ...
            <div className="modal-box max-w-lg bg-surface-card p-0 overflow-hidden dark:bg-surface-card-dark">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-surface-border dark:border-surface-border-dark">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 className="text-[14px] font-bold text-ink dark:text-ink-dark">{id ? 'Editar restricciones' : 'Registrar restricciones'}</h3>
                            <p className="text-[10px] text-ink-muted dark:text-ink-muted-dark">Alergias, intolerancias y alimentos a evitar</p>
                        </div>
                    </div>
