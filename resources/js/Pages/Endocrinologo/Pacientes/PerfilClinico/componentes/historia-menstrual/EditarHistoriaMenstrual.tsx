...
export default function EditarHistoriaMenstrual({ abierto, idPaciente, historia, onCerrar }: Props) {
    const { data, setData, post, processing, errors, reset } = useForm<FormData>({
        ...
        intervalo_entre_ciclos_dias: historia.intervalo_entre_ciclos_dias?.toString() ?? '',
        ...
        progesterona_lutea: historia.progesterona_lutea?.toString() ?? '',
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Editar registro menstrual</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <InputField label="Regularidad del ciclo" error={errors.regularidad_ciclo}>
                                <select value={data.regularidad_ciclo} onChange={(e) => setData('regularidad_ciclo', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark">
                            ...
                            <InputField label="Duración del ciclo (días)" error={errors.duracion_ciclo_dias}>
                                <input type="number" placeholder="Ej: 5" value={data.duracion_ciclo_dias} onChange={(e) => setData('duracion_ciclo_dias', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink placeholder:text-ink-muted/40 outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" />
                            </InputField>
                            <InputField label="Intervalo entre ciclos (días)">
                                <input type="number" placeholder="Ej: 28" value={data.intervalo_entre_ciclos_dias} onChange={(e) => setData('intervalo_entre_ciclos_dias', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink placeholder:text-ink-muted/40 outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" />
                            </InputField>
                            <InputField label="Fecha última menstruación">
                                <input type="date" value={data.fecha_ultima_menstruacion} onChange={(e) => setData('fecha_ultima_menstruacion', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark" />
                            </InputField>
                            <InputField label="Edad menarquía (años)">
                                <input type="number" placeholder="Ej: 12" value={data.edad_menarquia} onChange={(e) => setData('edad_menarquia', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink placeholder:text-ink-muted/40 outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" />
                            </InputField>
                            <InputField label="Progesterona lútea (ng/mL)">
                                <input type="number" step="0.01" placeholder="Ej: 12.5" value={data.progesterona_lutea} onChange={(e) => setData('progesterona_lutea', e.target.value)} className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink placeholder:text-ink-muted/40 outline-none focus:border-brand-green/50 focus:ring-0 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" />
                            </InputField>
                        </div>
                    </div>
