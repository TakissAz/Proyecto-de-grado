...
export default function CrearPerfilGonadotropo({ abierto, idPaciente, idConsulta, onCerrar }: Props) {
    const { data, setData, post, processing, reset } = useForm({
        id_consulta_endocrinologica: idConsulta ?? '',
        fecha_resultado: '',
        lh: '', fsh: '', relacion_lh_fsh: '',
        estradiol: '', progesterona: '',
        progesterona_dia_ciclo: '', progesterona_fase_ciclo: '',
        interpretacion: '',
    });

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(`/endocrinologo/pacientes/${idPaciente}/laboratorios/perfil-gonadotropo`, {
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Perfil gonadotropo</h2>
                            <p className="text-[11px] text-ink-muted dark:text-ink-muted-dark">Nuevo registro de gonadotropinas</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <InputLab label="Día del ciclo" unidad="" placeholder="Ej: 21" value={data.progesterona_dia_ciclo} onChange={(v) => setData('progesterona_dia_ciclo', v)} />
                        </div>
                    </div>
