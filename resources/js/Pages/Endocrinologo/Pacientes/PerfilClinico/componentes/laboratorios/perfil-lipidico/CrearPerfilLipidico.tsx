...
export default function CrearPerfilLipidico({ abierto, idPaciente, idConsulta, onCerrar }: Props) {
    const { data, setData, post, processing, reset } = useForm({
        id_consulta_endocrinologica: idConsulta ?? '',
        fecha_resultado: '',
        colesterol_total: '', hdl: '', ldl: '',
        vldl: '', trigliceridos: '', colesterol_no_hdl: '',
        dislipidemia_sugerida: false,
        interpretacion: '',
    });

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(`/endocrinologo/pacientes/${idPaciente}/laboratorios/perfil-lipidico`, {
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-category-others/15 text-category-others">
                            <FlaskConical size={18} strokeWidth={1.8} />
                        </div>
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Perfil lipídico</h2>
                            <p className="text-[11px] text-ink-muted dark:text-ink-muted-dark">Nuevo registro de lípidos y colesterol</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <InputLab label="Colesterol total" unidad="mg/dL" placeholder="<200" value={data.colesterol_total} onChange={(v) => setData('colesterol_total', v)} />
                            <InputLab label="HDL" unidad="mg/dL" placeholder=">50 (deseable)" value={data.hdl} onChange={(v) => setData('hdl', v)} />
                            <InputLab label="LDL" unidad="mg/dL" placeholder="<130" value={data.ldl} onChange={(v) => setData('ldl', v)} />
                            <InputLab label="VLDL" unidad="mg/dL" placeholder="<30" value={data.vldl} onChange={(v) => setData('vldl', v)} />
                            <InputLab label="Triglicéridos" unidad="mg/dL" placeholder="<150" value={data.trigliceridos} onChange={(v) => setData('trigliceridos', v)} />
                            <InputLab label="Col. no-HDL" unidad="mg/dL" placeholder="<130" value={data.colesterol_no_hdl} onChange={(v) => setData('colesterol_no_hdl', v)} />
                        </div>
                    </div>
