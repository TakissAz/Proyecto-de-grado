...
export default function EditarPerfilAndrogenico({ abierto, idPaciente, data: existente, onCerrar }: Props) {
    const { data, setData, post, processing, errors, reset } = useForm({
        ...
        hiperandrogenismo_bioquimico: existente.hiperandrogenismo_bioquimico,
        interpretacion: existente.interpretacion ?? '',
    });

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(`/endocrinologo/pacientes/${idPaciente}/laboratorios/perfil-androgenico/${existente.id_perfil_androgenico}?_method=PUT`, {
            preserveScroll: true,
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Editar perfil androgénico</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <InputLab label="Índice androgénico" unidad="" placeholder="<5" value={data.indice_androgenico_libre} onChange={(v) => setData('indice_androgenico_libre', v)} />
                    ...
                    <label className="flex items-start gap-3 cursor-pointer rounded-xl border border-surface-border px-4 py-3 transition-all hover:border-brand-green/30 dark:border-surface-border-dark dark:hover:border-brand-green/30">
                        ...
                        <div>
                            ...
                            <p className="text-[10.5px] text-ink-muted dark:text-ink-muted-dark">Al menos un andrógeno por encima del rango</p>
                        </div>
                    </label>
