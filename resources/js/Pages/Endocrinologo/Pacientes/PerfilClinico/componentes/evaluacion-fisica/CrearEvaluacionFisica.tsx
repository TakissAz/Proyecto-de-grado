...
export default function CrearEvaluacionFisica({ abierto, idPaciente, idConsulta, onCerrar }: Props) {
    const { data, setData, post, processing, reset } = useForm({
        ...
        acantosis_nigricans: false, skin_tags: false, galactorrea: false,
        hirsutismo_visible: false, puntaje_ferriman_gallwey: '',
        acne_visible: false, alopecia_visible: false,
        observaciones: '',
    });
            ...
            <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Nueva evaluación física</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <InputNum label="Peso (kg)" placeholder="Ej: 65" value={data.peso} onChange={(v) => setData('peso', v)} />
                            <InputNum label="Talla (m)" placeholder="Ej: 1.60" value={data.talla} onChange={(v) => setData('talla', v)} step="0.01" />
                            <InputNum label="IMC" placeholder="Auto" value={data.imc} onChange={(v) => setData('imc', v)} step="0.1" />
                            <InputNum label="Cintura (cm)" placeholder="Ej: 82" value={data.circunferencia_cintura} onChange={(v) => setData('circunferencia_cintura', v)} />
                            <InputNum label="Cadera (cm)" placeholder="Ej: 96" value={data.circunferencia_cadera} onChange={(v) => setData('circunferencia_cadera', v)} />
                            <InputNum label="Índice C/C" placeholder="Ej: 0.85" value={data.indice_cintura_cadera} onChange={(v) => setData('indice_cintura_cadera', v)} step="0.01" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3 max-w-xs">
                            <InputNum label="Sistólica (mmHg)" placeholder="Ej: 120" value={data.presion_sistolica} onChange={(v) => setData('presion_sistolica', v)} />
                            <InputNum label="Diastólica (mmHg)" placeholder="Ej: 80" value={data.presion_diastolica} onChange={(v) => setData('presion_diastolica', v)} />
                        </div>
                    </div>
