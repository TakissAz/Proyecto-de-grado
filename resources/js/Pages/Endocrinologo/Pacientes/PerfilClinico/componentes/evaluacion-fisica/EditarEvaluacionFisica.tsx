                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div>
                        <label className="flex items-center gap-2 mb-3 text-[11.5px] font-semibold text-ink-muted dark:text-ink-muted-dark">
                            <Scale size={11} strokeWidth={2} className="text-brand-green-dark dark:text-brand-green" /> Antropometría
                        </label>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <InputNum label="Peso (kg)" placeholder="65" value={data.peso} onChange={(v) => setData('peso', v)} />
                            <InputNum label="Talla (m)" placeholder="1.60" value={data.talla} onChange={(v) => setData('talla', v)} />
                            ...
                            <InputNum label="Índice C/C" placeholder="0.85" value={data.indice_cintura_cadera} onChange={(v) => setData('indice_cintura_cadera', v)} />
                        </div>
                    </div>

                    <div>
                        <label className="flex items-center gap-2 mb-3 text-[11.5px] font-semibold text-ink-muted dark:text-ink-muted-dark">
                            <Heart size={11} strokeWidth={2} className="text-category-fruits" /> Presión arterial
                        </label>
                    ...
                    <div>
                        <label className="flex items-center gap-2 mb-3 text-[11.5px] font-semibold text-ink-muted dark:text-ink-muted-dark">
                            <Eye size={11} strokeWidth={2} className="text-brand-orange" /> Hallazgos al examen
                        </label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            ...
                            <CheckboxItem label="Acrocordones" descripcion="Fibromas blandos" checked={data.skin_tags} onChange={(v) => setData('skin_tags', v)} />
                    ...
                    <div>
                        ...
                        <textarea rows={3} value={data.observaciones} onChange={(e) => setData('observaciones', e.target.value)} placeholder="Notas..." className="w-full rounded-xl border border-surface-border bg-[#FAF9F6] px-4 py-3 text-[13px] text-ink placeholder:text-ink-muted/40 outline-none focus:border-brand-green/50 focus:ring-0 resize-y min-h-[80px] dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark" />
                    </div>
...
function InputNum({ label, placeholder, value, onChange }: { label: string; placeholder: string; value: string; onChange: (v: string) => void }) {
    return (
        <div>
