...
import { Activity, Edit, Plus, History, Calendar, Scale, Heart } from 'lucide-react';
...
export default function TarjetaEvaluacionFisica({ evaluacion, idPaciente, onRegistrar, onEditar }: Props) {
        ...
        return (
            <div className="p-5">
                ...
                <p className="text-[12px] text-ink-muted dark:text-ink-muted-dark mb-4 leading-relaxed">
                    No se ha registrado la evaluación física. Estos datos son necesarios para evaluar riesgo metabólico.
                </p>
    ...
    if (evaluacion.imc != null && evaluacion.imc >= 25) alertas.push(`IMC ${evaluacion.imc >= 30 ? 'Obesidad' : 'Sobrepeso'}`);
    if (evaluacion.circunferencia_cintura != null && evaluacion.circunferencia_cintura >= 80) alertas.push('Cintura ≥ 80');
    if (evaluacion.presion_sistolica != null && evaluacion.presion_sistolica >= 130) alertas.push('PA elevada');
    if (evaluacion.acantosis_nigricans) alertas.push('Acantosis');
    ...
    return (
        <div className="p-5 space-y-4">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2.5">
                    <div className={clsx('flex h-8 w-8 items-center justify-center rounded-xl', tieneHallazgos ? 'bg-brand-orange/15' : 'bg-brand-green/15')}>
                        <Activity size={15} strokeWidth={1.8} className={tieneHallazgos ? 'text-brand-orange' : 'text-brand-green-dark dark:text-brand-green'} />
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <Link href={`/endocrinologo/pacientes/${idPaciente}/evaluacion-fisica/historial`} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-ink-muted transition-colors hover:bg-black/[0.03] hover:text-ink dark:text-ink-muted-dark dark:hover:bg-white/[0.04] dark:hover:text-ink-dark">
            ...
            {/* Gráfico IMC + Mediciones */}
            <div className="grid grid-cols-1 sm:grid-cols-[160px_1fr] gap-4">
                {evaluacion.imc != null && <GraficoIMC imc={evaluacion.imc} />}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        ...
                        <DatoItem label="IMC" valor={evaluacion.imc != null ? `${evaluacion.imc}` : null} destacar={evaluacion.imc != null && evaluacion.imc >= 25} />
                        <DatoItem label="Cintura" valor={evaluacion.circunferencia_cintura != null ? `${evaluacion.circunferencia_cintura} cm` : null} destacar={evaluacion.circunferencia_cintura != null && evaluacion.circunferencia_cintura >= 80} />
                        <DatoItem label="Cadera" valor={evaluacion.circunferencia_cadera != null ? `${evaluacion.circunferencia_cadera} cm` : null} />
                        <DatoItem label="ICC" valor={evaluacion.indice_cintura_cadera != null ? `${evaluacion.indice_cintura_cadera}` : null} destacar={evaluacion.indice_cintura_cadera != null && evaluacion.indice_cintura_cadera >= 0.85} />
                    </div>
                </div>
            </div>
            ...
            {(evaluacion.presion_sistolica || evaluacion.presion_diastolica) && (
                <div>
                    <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted-dark mb-2">
                        <Heart size={10} strokeWidth={2} className="text-category-fruits" />
                        Presión arterial
                    </p>
                    <div className="grid grid-cols-2 gap-2 max-w-xs">
                        <DatoItem label="Sistólica" valor={evaluacion.presion_sistolica != null ? `${evaluacion.presion_sistolica} mmHg` : null} destacar={evaluacion.presion_sistolica != null && evaluacion.presion_sistolica >= 130} />
                        <DatoItem label="Diastólica" valor={evaluacion.presion_diastolica != null ? `${evaluacion.presion_diastolica} mmHg` : null} destacar={evaluacion.presion_diastolica != null && evaluacion.presion_diastolica >= 85} />
                    </div>
                </div>
            )}
            ...
            {alertas.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                    {alertas.map(a => <Badge key={a} color="orange">{a}</Badge>)}
                </div>
            )}

            {/* Interpretación */}
            <p className={clsx('text-[12px] font-medium', tieneHallazgos ? 'text-brand-orange' : 'text-ink-muted dark:text-ink-muted-dark')}>
                {tieneHallazgos ? 'Existen hallazgos físicos relevantes para riesgo metabólico.' : 'Sin hallazgos relevantes.'}
            </p>
            ...
            {evaluacion.observaciones && (
                <div className="rounded-xl border border-surface-border px-4 py-3 dark:border-surface-border-dark">
                    ...
                    <p className="text-[12.5px] text-ink dark:text-ink-dark leading-relaxed">{evaluacion.observaciones}</p>
                </div>
            )}
        </div>
    );
