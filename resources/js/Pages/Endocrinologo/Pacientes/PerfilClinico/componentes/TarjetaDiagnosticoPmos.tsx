...
interface Props { evaluacion: EvaluacionPmosData; diagnostico: DiagnosticoPmosData | null; idPaciente: number; onRegistrar: () => void; onEditar: () => void; }
    ...
    return <Tarjeta>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-base-content">Diagnóstico PMOS</h3>
                {diagnostico ? <Badge variante={diagnostico.diagnostico_confirmado ? 'error' : 'ghost'}>{diagnostico.diagnostico_confirmado ? 'Confirmado' : 'No confirmado'}</Badge> : <Badge>Pendiente de evaluación</Badge>}
            </div>
            {diagnostico ? <div className="flex flex-wrap gap-2">
                <a href={`/endocrinologo/pacientes/${idPaciente}/diagnostico/pmos/reporte-pdf`} target="_blank" rel="noreferrer" className="btn btn-outline btn-secondary btn-xs gap-1"><Download size={13} /> Descargar PDF PMOS</a>
        ...
        {diagnostico && <>
            ...
            <div className="space-y-3">
                {validacionResuelta ? <div className="alert alert-info py-2 text-xs"><span>La valoración asistida ya fue revisada por el especialista.</span></div> : !tieneAnalisis ? <BotonEjecutarSistemaExperto url={`/endocrinologo/sistema-experto/pmos/${diagnostico.id_diagnostico_pmos}/ejecutar`} label="Generar análisis clínico PMOS" successMessage="El análisis clínico PMOS fue generado correctamente." onSuccess={() => router.reload({ only: ['perfil'] })} /> : null}
                ...
                {tieneAnalisis && mostrarResultados && <div className="space-y-3 rounded-2xl border border-primary/15 bg-base-200/30 p-3">
                    ...
                    <ValidacionResultadoExperto url={`/endocrinologo/sistema-experto/pmos/${diagnostico.id_diagnostico_pmos}/validar`} estadoActual={diagnostico.estado_validacion_experta} validadoPor={diagnostico.validado_por} fechaValidacion={diagnostico.fecha_validacion} observacionActual={diagnostico.observacion_validacion} onRejected={() => setEditandoClinico(true)} onSuccess={(data) => { const estado = (data as { estado_validacion_experta?: string } | undefined)?.estado_validacion_experta; if (estado !== 'rechazado') router.reload({ only: ['perfil'] }); }} />
                </div>}
            </div>
        </>}
        {diagnostico && <FormularioEditarDiagnosticoPmos abierto={editandoClinico} diagnostico={diagnostico} evaluacion={evaluacion} onCerrar={() => setEditandoClinico(false)} onSuccess={() => router.reload({ only: ['perfil'] })} />}
    </Tarjeta>;
}
