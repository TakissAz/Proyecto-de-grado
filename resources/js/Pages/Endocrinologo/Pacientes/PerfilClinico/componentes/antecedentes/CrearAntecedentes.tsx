...
export default function CrearAntecedentes({ abierto, idPaciente, idConsulta, onCerrar }: Props) {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(`/endocrinologo/pacientes/${idPaciente}/antecedentes`, {
            preserveScroll: true,
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-surface-border bg-surface-card shadow-2xl dark:border-surface-border-dark dark:bg-surface-card-dark">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-surface-border bg-surface-card px-6 py-4 dark:border-surface-border-dark dark:bg-surface-card-dark">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[16px] font-bold text-ink dark:text-ink-dark">Nuevo registro de antecedentes</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <SeccionPersonales data={data} setData={setData} />

                    {/* Antecedentes familiares */}
                    <SeccionFamiliares data={data} setData={setData} />

                    {/* Medicamentos */}
                    <SeccionMedicamentos data={data} setData={setData} />
...
function SeccionPersonales({ data, setData }: { data: any; setData: (key: string, val: any) => void }) {
    return (
        <div>
...
function SeccionFamiliares({ data, setData }: { data: any; setData: (key: string, val: any) => void }) {
    return (
        <div>
            <label className="flex items-center gap-2 mb-3 text-[11.5px] font-semibold text-ink-muted dark:text-ink-muted-dark">
...
function SeccionMedicamentos({ data, setData }: { data: any; setData: (key: string, val: any) => void }) {
    return (
        <div>
            <label className="flex items-center gap-2 mb-3 text-[11.5px] font-semibold text-ink-muted dark:text-ink-muted-dark">
