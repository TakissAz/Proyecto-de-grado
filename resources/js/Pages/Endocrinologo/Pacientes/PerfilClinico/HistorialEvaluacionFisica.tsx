...
export default function HistorialEvaluacionFisica({ paciente, registros }: Props) {
    ...
    const registrosFiltrados = useMemo(() => {
        return registros.filter((r) => {
            ...
            if (imc) {
                ...
                if (imc === 'normal' && v >= 25) return false;
                if (imc === 'sobrepeso' && (v < 25 || v >= 30)) return false;
                if (imc === 'obesidad' && v < 30) return false;
            }
    ...
    }, [registros, signo, imc, desde, hasta]);
    ...
    const urlPdf = useMemo(() => {
        ...
        return `/endocrinologo/pacientes/${id}/evaluacion-fisica/reporte-pdf${qs ? `?${qs}` : ''}`;
    }, [id, signo, imc, desde, hasta]);
                    ...
                    <div className="card-elevated p-4">
                        <div className="flex flex-wrap items-end gap-3">
                            <div className="flex-1 min-w-[150px]">
                                <label className="text-[10px] font-semibold uppercase tracking-wide text-ink-muted dark:text-ink-muted-dark mb-1 block">Signo clínico</label>
                                <select value={signo} onChange={(e) => { setSigno(e.target.value); setPagina(1); }}
                                    ...
                                    <option value="acantosis_nigricans">Acantosis nigricans</option>
                                    <option value="skin_tags">Acrocordones</option>
                                    <option value="hirsutismo_visible">Hirsutismo</option>
                                    <option value="acne_visible">Acné</option>
                                    <option value="alopecia_visible">Alopecia</option>
                                    <option value="galactorrea">Galactorrea</option>
                                </select>
                            </div>
                            <div className="flex-1 min-w-[130px]">
                                <label className="text-[10px] font-semibold uppercase tracking-wide text-ink-muted dark:text-ink-muted-dark mb-1 block">Rango IMC</label>
                                <select value={imc} onChange={(e) => { setImc(e.target.value); setPagina(1); }}
                                    className="w-full rounded-lg border border-surface-border bg-[#FAF9F6] px-3 py-2 text-[12px] text-ink outline-none focus:border-brand-green/50 dark:border-surface-border-dark dark:bg-[#20232B] dark:text-ink-dark">
                                    <option value="">Todos</option>
                                    <option value="normal">Normal (&lt;25)</option>
                                    <option value="sobrepeso">Sobrepeso (25–29.9)</option>
                                    <option value="obesidad">Obesidad (≥30)</option>
                                </select>
                            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-4">
                        ...
                        <div className="space-y-3">
                            ...
                            {registrosFiltrados.length === 0 ? (
                                ...
                                <RegistroCard key={r.id_evaluacion_fisica} registro={r} numero={registrosFiltrados.length - ((pagina - 1) * porPagina + idx)} />
                            ))}
